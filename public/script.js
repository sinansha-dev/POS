const state = {
  ...
  stockBatches: [],
  reports: {},
  localSuppliers: JSON.parse(localStorage.getItem("novapos_local_suppliers") || "[]"),
  localCustomers: JSON.parse(localStorage.getItem("novapos_local_customers") || "[]")
};
...
function persistLocalModuleData() {
  localStorage.setItem("novapos_local_suppliers", JSON.stringify(state.localSuppliers));
  localStorage.setItem("novapos_local_customers", JSON.stringify(state.localCustomers));
}
...
async function addSupplier(event) {
  ...
  await loadBootstrap();
    ...
    if (sqliteOnlyFeatureNotice(error)) {
      const supplier = {
        id: `local-${Date.now()}`,
        name: document.getElementById('supplierName').value,
        phone: document.getElementById('supplierPhone').value,
      ...
      state.localSuppliers.unshift(supplier);
      persistLocalModuleData();
      event.target.reset();
      await loadBootstrap();
      alert("Saved locally. It will sync once backend module is available.");
      return;
    }
...
async function addCustomer(event) {
  ...
  await loadBootstrap();
    ...
    if (sqliteOnlyFeatureNotice(error)) {
      ...
      state.localCustomers.unshift(customer);
      persistLocalModuleData();
      event.target.reset();
      await loadBootstrap();
      alert("Saved locally. It will sync once backend module is available.");

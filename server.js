...
function isSupabaseMissingColumnError(err) {
  ...
  async insertSale(sale) {
    if (SUPABASE_URL) {
      try {
        ...
        const rows = await sbQuery("sales", "POST", fallback);
        return rows?.[0]?.id;
      }
    }
  ...
  async getProductFull(id) {
    if (SUPABASE_URL) {
      try {
        ...
        if (!isSupabaseMissingColumnError(e)) throw e;
        const rows = await sbQuery("products", "GET", null,
          `?id=eq.${id}&select=id,name,sku,price,gst_rate,hsn_code,stock&limit=1`);
        const p = rows?.[0] || null;
        if (!p) return null;
        return {
          ...p,
